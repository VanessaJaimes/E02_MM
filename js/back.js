function Calc_per(percent, percent1) {
  ...
  if (percent1 < 40) {
    document.getElementById("myProgressBar1").className = "progress-bar progress-bar-striped progress-bar-animated bg-danger";
  } else if (percent1 < 70) {
    document.getElementById("myProgressBar1").className = "progress-bar progress-bar-striped progress-bar-animated bg-warning";
  } else {
    document.getElementById("myProgressBar1").className = "progress-bar progress-bar-striped progress-bar-animated bg-success";
  }
}
/*
...
function Init() {
  ...
  Calc_per(JSON.parse(localStorage.getItem('Percent'))[0],
...
var app = new Vue({
  ...
  data: {
    ...
    Percent: [100, 100],
    Sales: [],
    Whi: [JSON.parse(localStorage.getItem('Warehouse'))[0],
  ...
  methods: {
    Buy() {
      ...
      if (this.Whi[this.Warehouse] - amount < 0) {
        ...
        this.Percent[this.Warehouse] = this.Whi[this.Warehouse] * 100 / JSON.parse(localStorage.getItem('Warehouse'))[this.Warehouse]
        this.Sales.push({
          Ware: this.Warehouse,
          Buy: amount,
        })
    ...
    Alert() {
      (this.Percent[this.Warehouse] <= 50) ?
        (this.Percent[this.Warehouse] <= 10) ?
          this.Message = "el almacenamiento es inferior al 10% para la bodega " + (this.Warehouse + 1) :
          this.Message = "el almacenamiento es inferior al 50% para la bodega " + (this.Warehouse + 1)
        :
        this.Message = ""
    }
  }
